...
def load_variants_file(path: Path, column: int) -> dict:
    ...
    Returns {"male": {base: variant}, "female": {base: variant}}.
    """
    male, female = {}, {}
    current = male
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if stripped.startswith("#"):
                ...
                if "FEMININ" in upper or "FEMALE" in upper:
                    current = female
                continue
...
def assemble_mod(output_root: Path, processed: list[tuple[dict, Path]]) -> Path:
    """Assemble a mod-ready directory from per-language outputs.

    Creates output/mod/ with the full submod structure:
...
def main():
    ...
    assemble_mod(output_root, processed)
