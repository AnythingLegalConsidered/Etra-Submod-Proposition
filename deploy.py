...
Runs generate_mod.py --all, then copies output/mod/ to the Paradox mod folder.
...
def deploy(root: Path, mod_dir: Path, dry_run: bool = False) -> bool:
    """Copy output/mod/ contents to the Paradox mod folder."""
    source = root / "output" / "mod"
    ...
    if not files:
        print("WARNING: No files found in output/mod/", file=sys.stderr)
        return False

    print(f"\nDeploying {len(files)} files to {mod_dir}")
...
def main():
    ...
    parser.add_argument("--mod-dir", type=Path, default=DEFAULT_MOD_DIR, help="Target mod directory")
